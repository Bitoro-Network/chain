...
// NewRootCmd creates a new root command for `bitoroprotocold`. It is called once in the main function.
...
func NewRootCmd(
	...
	return NewRootCmdWithInterceptors(
		...
		func(s string, appConfig *BitoroAppConfig) (string, *BitoroAppConfig) {
			return s, appConfig
		},
		func(app *bitoroapp.App) *bitoroapp.App {
			return app
		},
	)
}
...
func NewRootCmdWithInterceptors(
	...
	appConfigInterceptor func(string, *BitoroAppConfig) (string, *BitoroAppConfig),
	appInterceptor func(app *bitoroapp.App) *bitoroapp.App,
) *cobra.Command {
	initAppOptions := viper.New()
	initAppOptions.Set(flags.FlagHome, tempDir())
	tempApp := bitoroapp.New(
		log.NewNopLogger(),
		dbm.NewMemDB(),
		nil,
		true,
	...
	rootCmd := &cobra.Command{
		...
		Short: "Start bitoroprotocol app",
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			...
			if ddErrorTrackingFormatterEnabled :=
				...
				if enabled, err := cast.ToBoolE(ddErrorTrackingFormatterEnabled); err == nil && enabled {
					bitoroapp.SetZerologDatadogErrorTrackingFormat()
				}
			}
...
func initRootCmd(
	...
	rootCmd.AddCommand(
		genutilcli.InitCmd(tempApp.ModuleBasics, bitoroapp.DefaultNodeHome),
		genutilcli.CollectGenTxsCmd(
			banktypes.GenesisBalancesIterator{},
			bitoroapp.DefaultNodeHome,
		...
		AddGenesisAccountCmd(bitoroapp.DefaultNodeHome),
...
// autoCliOpts returns options based upon the modules in the Bitoro app.
//
// Creates an instance of the application that is discarded to enumerate the modules.
func autoCliOpts(tempApp *bitoroapp.App, initClientCtx client.Context) autocli.AppOptions {
...
func appExport(
	...
	if height != -1 {
		if err := bitoroApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	}

	return bitoroApp.ExportAppStateAndValidators(forZeroHeight, jailAllowedAddrs, modulesToExport)
}

var tempDir = func() string {
	dir, err := os.MkdirTemp("", "bitoroprotocol")
	if err != nil {
		dir = bitoroapp.DefaultNodeHome
	}
