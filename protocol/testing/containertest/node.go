...
func (n *Node) getContextForBroadcastTx(signer string) (*client.Context, *pflag.FlagSet, error) {
	...
	if err := flags.Set("chain-id", "localbitoroprotocol"); err != nil {
		return nil, nil, err
	}

	// NB: In `cmd/bitoroprotocol/root.go` this step is done before ReadFromClientConfig, but here we choose to
