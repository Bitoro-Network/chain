...
import (
	...
	"github.com/Bitoro-Network/chain/protocol/testing/version"
	"github.com/Bitoro-Network/chain/protocol/testutil/constants"
	pricefeed_testutil "github.com/Bitoro-Network/chain/protocol/testutil/pricefeed"
...
func (t *Testnet) initializeNode(moniker string) (*Node, error) {
	...
	if t.isPreupgradeGenesis {
		entrypointCommand = "/bitoroprotocol/preupgrade_entrypoint.sh"
	} else {
		entrypointCommand = "bitoroprotocold"
	}

	resource, err := t.pool.RunWithOptions(
		&dockertest.RunOptions{
			Name:       fmt.Sprintf("testnet-local-%s", moniker),
			Repository: "bitoroprotocol-container-test",
			Tag:        "",
			...
			Entrypoint: []string{
				...
				"start",
				"--home",
				fmt.Sprintf("/bitoroprotocol/chain/.%s", moniker),
			...
			Env: []string{
				"DAEMON_NAME=bitoroprotocold",
				fmt.Sprintf("DAEMON_HOME=/bitoroprotocol/chain/.%s", moniker),
