...
SNAP_PATH="/bitoroprotocol/chain/.full-node-0/snapshots/bitoroprotocol/"
# logfile containing snapshot timestamps
LOG_PATH="/bitoroprotocol/chain/.full-node-0/snapshots/bitoroprotocol/bitoroprotocol_log.txt"
# data directory to snapshot. this contains the blockchain state.
DATA_PATH="/bitoroprotocol/chain/.full-node-0/data/"
...
touch $LOG_PATH
sleep 10
CHAIN_ID="bitoro-testnet-1"

# Prune snapshots to prevent them from getting too big. We make 3 changes:
# Prune all app state except last 2 blocks
sed -i 's/pruning = "default"/pruning = "everything"/' /bitoroprotocol/chain/.full-node-0/config/app.toml
...
sed -i 's/min-retain-blocks = 0/min-retain-blocks = 2/' /bitoroprotocol/chain/.full-node-0/config/app.toml
# Do not index tx_index.db
sed -i 's/indexer = "kv"/indexer = "null"/' /bitoroprotocol/chain/.full-node-0/config/config.toml
...
while true; do
  ...
  cosmovisor run start --log_level info --home /bitoroprotocol/chain/.full-node-0 --p2p.seeds "${p2p_seeds}" --non-validating-full-node=true --dd-agent-host=${dd_agent_host} &
