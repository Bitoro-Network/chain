...
set_cosmovisor_binary_permissions() {
    ...
    for version in "${!version_to_url[@]}"; do
        ...
        mv "$version_dir/build/$binary_file" "$version_dir/bin/bitoroprotocold"
        chmod 755 "$version_dir/bin/bitoroprotocold"
        echo "Successfully set up $version_dir/bin/bitoroprotocold"
    done
    current_version_path="$HOME/cosmovisor/upgrades/$CURRENT_VERSION/bin"
    mkdir -p $current_version_path
    cp /bin/bitoroprotocold $current_version_path
}
...
# Modify any of the default configuration generated during `bitoroprotocold init`. For seed nodes
