...
SNAP_PATH="/bitoroprotocol/chain/.full-node-2/snapshots/bitoroprotocol/"
# logfile containing snapshot timestamps
LOG_PATH="/bitoroprotocol/chain/.full-node-2/snapshots/bitoroprotocol/bitoroprotocol_log.txt"
# data directory to snapshot. this contains the blockchain state.
DATA_PATH="/bitoroprotocol/chain/.full-node-2/data/"
...
sed -i 's/pruning = "default"/pruning = "everything"/' /bitoroprotocol/chain/.full-node-2/config/app.toml
...
sed -i 's/min-retain-blocks = 0/min-retain-blocks = 2/' /bitoroprotocol/chain/.full-node-2/config/app.toml
# Do not index tx_index.db
sed -i 's/indexer = "kv"/indexer = "null"/' /bitoroprotocol/chain/.full-node-2/config/config.toml
...
while true; do
  ...
  cosmovisor run start --log_level info --home /bitoroprotocol/chain/.full-node-2 --p2p.seeds "${p2p_seeds}" --non-validating-full-node=true --dd-agent-host=${dd_agent_host} &
