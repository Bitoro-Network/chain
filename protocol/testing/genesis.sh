...
SUBACCOUNTS_MODACC_ADDR="btoro1v88c3xv9xyv3eetdx0tvcmq7ung3dywp5upwc6"
REWARDS_VESTER_ACCOUNT_ADDR="btoro1ltyc6y4skclzafvpznpt2qjwmfwgsndp458rmp"
# Address of the `bridge` module account.
# Obtained from `authtypes.NewModuleAddress(bridgetypes.ModuleName)`.
BRIDGE_MODACC_ADDR="btoro1zlefkpe3g0vvm9a4h0jf9000lmqutlh9jwjnsv"
...
MEGAVAULT_MAIN_VAULT_ACCOUNT_ADDR="btoro18tkxrnrkqc2t0lr3zxr5g6a4hdvqksylxqje4r"
DEFAULT_MEGAVAULT_MAIN_VAULT_QUOTE_BALANCE=0 # 0 USDC
NATIVE_TOKEN_WHOLE_COIN="dv4tnt"
COIN_NAME="Bitoro Testnet Token"
...
function edit_genesis() {
    ...
    # Marketmap: BTORO-USD
    dasel put -t json -f "$GENESIS" '.app_state.marketmap.market_map.markets.BTORO/USD' -v "{}"
    dasel put -t json -f "$GENESIS" '.app_state.marketmap.market_map.markets.BTORO/USD.ticker' -v "{}" 

    dasel put -t json -f "$GENESIS" '.app_state.marketmap.market_map.markets.BTORO/USD.ticker.currency_pair' -v "{}"
    dasel put -t string -f "$GENESIS" '.app_state.marketmap.market_map.markets.BTORO/USD.ticker.currency_pair.Base' -v 'BTORO'
    dasel put -t string -f "$GENESIS" '.app_state.marketmap.market_map.markets.BTORO/USD.ticker.currency_pair.Quote' -v 'USD'

    dasel put -t int -f "$GENESIS" '.app_state.marketmap.market_map.markets.BTORO/USD.ticker.decimals' -v '9'
    dasel put -t int -f "$GENESIS" '.app_state.marketmap.market_map.markets.BTORO/USD.ticker.min_provider_count' -v '3'
    dasel put -t bool -f "$GENESIS" '.app_state.marketmap.market_map.markets.BTORO/USD.ticker.enabled' -v 'true'

    dasel put -t json -f "$GENESIS" '.app_state.marketmap.market_map.markets.BTORO/USD.provider_configs.[]' -v '{"name": "binance_ws", "off_chain_ticker": "BTOROUSDT", "normalize_by_pair": {"Base": "USDT", "Quote": "USD"}}'
    dasel put -t json -f "$GENESIS" '.app_state.marketmap.market_map.markets.BTORO/USD.provider_configs.[]' -v '{"name": "bybit_ws", "off_chain_ticker": "BTOROUSDT", "normalize_by_pair": {"Base": "USDT", "Quote": "USD"}}'
    dasel put -t json -f "$GENESIS" '.app_state.marketmap.market_map.markets.BTORO/USD.provider_configs.[]' -v '{"name": "gate_ws", "off_chain_ticker": "BTORO_USDT", "normalize_by_pair": {"Base": "USDT", "Quote": "USD"}}'
    dasel put -t json -f "$GENESIS" '.app_state.marketmap.market_map.markets.BTORO/USD.provider_configs.[]' -v '{"name": "kucoin_ws", "off_chain_ticker": "BTORO-USDT", "normalize_by_pair": {"Base": "USDT", "Quote": "USD"}}'
    dasel put -t json -f "$GENESIS" '.app_state.marketmap.market_map.markets.BTORO/USD.provider_configs.[]' -v '{"name": "okx_ws", "off_chain_ticker": "BTORO-USDT", "normalize_by_pair": {"Base": "USDT", "Quote": "USD"}}'
	...
	# Market: BTORO-USD
	dasel put -t json -f "$GENESIS" '.app_state.prices.market_params.[]' -v "{}"
	dasel put -t string -f "$GENESIS" '.app_state.prices.market_params.[34].pair' -v 'BTORO-USD'
	...
	dasel put -t int -f "$GENESIS" '.app_state.prices.market_prices.[34].price' -v '2050000000'          # $2.05 = 1 BTORO.
  ...
  # Schedule a delayed message to swap fee tiers to the standard schedule after ~120 days of blocks.
	...
	# Set operator params.
	dasel put -t string -f "$GENESIS" ".app_state.vault.operator_params.operator" -v 'btoro10d07y265gmmuvt4z0w9aw880jnsr700jnmapky'
	...
	if [ -z "${INPUT_TEST_ACCOUNTS[0]}" ]; then
		vault_owner_address='btoro199tqg4wdlnu4qjlxchpd7seg454937hjrknju4' # alice as default vault owner
	else
	...
	dasel put -t bool -f "$GENESIS" '.app_state.bitoroaccountplus.params.is_smart_account_active' -v 'true'
}
