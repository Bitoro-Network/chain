...
bitoroprotocold init tmp --chain-id bitoroprotocol --home ./tmp-new-validator >/dev/null 2>&1

# Take the last line of the output which contains the new mnemonic
mnemonic=$(bitoroprotocold keys add val --home ./tmp-new-validator --keyring-backend=test 2>&1 >/dev/null | tail -1)

# Overwrite the randomly generated `priv_validator_key.json` with a key generated deterministically from the mnemonic.
bitoroprotocold tendermint gen-priv-key --home ./tmp-new-validator --mnemonic "$mnemonic"
address=$(bitoroprotocold keys show val --address --keyring-backend=test --home ./tmp-new-validator)
node_key=$(jq -r ".priv_key.value" ./tmp-new-validator/config/node_key.json)
node_id=$(bitoroprotocold tendermint show-node-id --home ./tmp-new-validator)
cons_address=$(bitoroprotocold tendermint show-address --home ./tmp-new-validator)
