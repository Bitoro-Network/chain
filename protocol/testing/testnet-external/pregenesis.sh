...
# The pregenesis file includes initial genesis module states for the testnet, as well as two internal validators (bitoro-1 and bitoro-2) run by the Bitoro team.
...
#
# example usage:
# $ ./testing/testnet-external/pregenesis.sh ./build/bitoroprotocold --SEED_FAUCET_USDC

# To get the following information, first set up the validator keys locally. Then run:
# Account address: `bitoroprotocold keys show bitoro-1-key -a`
# Consensus address: `bitoroprotocold tendermint show-address`
# Node ID: `bitoroprotocold tendermint show-node-id`

# Check for missing required arguments
if [ -z "$1" ]; then
  echo "Error: Missing required argument BITORO_BINARY."
  echo "Usage: $0 <BITORO_BINARY> [-s|--SEED_FAUCET_USDC]"
  exit 1
fi
...
CHAIN_ID="bitoro-testnet-4"
FAUCET_ACCOUNTS=(
	"btoro1g2ygh8ufgwwpg5clp2qh3tmcmlewuyt2z6px8k" # main faucet
	"btoro1fzhzmcvcy7nycvu46j9j4f7f8cnqxn3770q260" # backup #1
	"btoro1xeu4caf7nwd83h9z49cxtagsglngdldjgtrzfq" # backup #2
)
...
# This also controls in which directory the validator's home will be located. i.e. `/tmp/chain/.bitoro-1`
MONIKERS=(
	"bitoro-1"
	"bitoro-2"
	"bitoro-research"
...
IPS=(
	"3.20.153.106" # bitoro-1, us-east-2
	"18.182.95.191" # bitoro-2, ap-northeast-1
	"3.139.127.183" # bitoro-research, us-east-2
)

MNEMONICS_SECRET="$(AWS_PROFILE=bitoro-public-testnet aws secretsmanager get-secret-value --region $AWS_REGION --secret-id public-testnet-mnemonics | jq -r '.SecretString')"

RESEARCH_MNEMONICS_SECRET="$(AWS_PROFILE=bitoro-research aws secretsmanager get-secret-value --region $AWS_REGION --secret-id public-testnet-mnemonics | jq -r '.SecretString')"

# Define mnemonics for internal validators.
MNEMONICS=(
	# bitoro-1
	# Consensus Address: btorovalcons18an8qvxam8zkrmrx7d0gygd7q9uv7cky7jpq5x
	"$(echo $MNEMONICS_SECRET | jq -r '.["bitoro-1"]')"

	# bitoro-2
	# Consensus Address: btorovalcons1z79h40nmd777scs93qjxaeak8m2cl6hpqg2rx9
	"$(echo $MNEMONICS_SECRET | jq -r '.["bitoro-2"]')"

	# bitoro-research
	# Consensus Address: btorovalcons1a49fhxhy7mn64v220v5wgpyauwzdc4y8rej9xh
	"$(echo $RESEARCH_MNEMONICS_SECRET)"
)

NODE_KEYS_SECRET="$(AWS_PROFILE=bitoro-public-testnet aws secretsmanager get-secret-value --region $AWS_REGION --secret-id public-testnet-node-keys | jq -r '.SecretString')"
RESEARCH_NODE_KEYS_SECRET="$(AWS_PROFILE=bitoro-research aws secretsmanager get-secret-value --region $AWS_REGION --secret-id public-testnet-node-keys | jq -r '.SecretString')"

# Define node keys for internal validators.
NODE_KEYS=(
	# Node ID: 3f667030ddd9c561ec66f35e8221be0178cf62c4
	"$(echo $NODE_KEYS_SECRET | jq -r '.["bitoro-1"]')"

	# Node ID: 178b7abe7b6fbde8620588246ee7b63ed58feae1
	"$(echo $NODE_KEYS_SECRET | jq -r '.["bitoro-2"]')"

	"$(echo $RESEARCH_NODE_KEYS_SECRET)"
)

VALIDATOR_ACCOUNTS=(
	"btoro1vvc9vl6z9pu0vt2y79d0ln8zp6qmpmrhrcnnuy" # bitoro-1
	"btoro10lzv79d96l7jh07z76ry6cnn6ftnnl8fdg0afd" # bitoro-2
	"btoro1md63arq56n623g5xpevev94lyepv4pqjjs6y74" # bitoro-research
)
...
create_pregenesis_file() {
	VAL_HOME_DIR="$TMP_CHAIN_DIR/.bitoroprotocol"
	VAL_CONFIG_DIR="$VAL_HOME_DIR/config"

	# This initializes the $VAL_HOME_DIR folder.
	$BITORO_BINARY init "test-moniker" -o --chain-id=$CHAIN_ID --home "$VAL_HOME_DIR"
	...
	for acct in "${FAUCET_ACCOUNTS[@]}"; do
		$BITORO_BINARY add-genesis-account "$acct" $FAUCET_BALANCE --home "$VAL_HOME_DIR"
	done
	...
	for i in "${!MONIKERS[@]}"; do
		...
		$BITORO_BINARY init "${MONIKERS[$i]}" -o --chain-id=$CHAIN_ID --home "$INDIVIDUAL_VAL_HOME_DIR"

		# Overwrite the randomly generated `priv_validator_key.json` with a key generated deterministically from the mnemonic.
		$BITORO_BINARY tendermint gen-priv-key --home "$INDIVIDUAL_VAL_HOME_DIR" --mnemonic "${MNEMONICS[$i]}"
		...
		echo "${MNEMONICS[$i]}" | $BITORO_BINARY keys add "${MONIKERS[$i]}" --recover --keyring-backend=test --home "$INDIVIDUAL_VAL_HOME_DIR"
		...
		$BITORO_BINARY add-genesis-account "${VALIDATOR_ACCOUNTS[$i]}" "${TESTNET_VALIDATOR_NATIVE_TOKEN_BALANCE}$NATIVE_TOKEN" --home "$INDIVIDUAL_VAL_HOME_DIR"

		# Initialize the validator account in `genesis.json` under the common home directory, which is used as the output geneis file.
		$BITORO_BINARY add-genesis-account "${VALIDATOR_ACCOUNTS[$i]}" "${TESTNET_VALIDATOR_NATIVE_TOKEN_BALANCE}$NATIVE_TOKEN" --home "$VAL_HOME_DIR"

		$BITORO_BINARY gentx "${MONIKERS[$i]}" "${TESTNET_VALIDATOR_SELF_DELEGATE_AMOUNT}$NATIVE_TOKEN" --moniker="${MONIKERS[$i]}" --keyring-backend=test --chain-id=$CHAIN_ID --home "$INDIVIDUAL_VAL_HOME_DIR" --ip="${IPS[$i]}"
	...
	$BITORO_BINARY collect-gentxs --home "$VAL_HOME_DIR"
}
