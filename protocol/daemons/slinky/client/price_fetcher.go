...
// FetchPrices pulls prices from Slinky, translates the returned data format to bitoro-compatible types,
// and sends the price updates to the index price cache via the pricefeed server.
// It uses the MarketPairFetcher to efficiently map between Slinky's CurrencyPair primary key and bitoro's
