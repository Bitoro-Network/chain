...
import (
	...
	ratelimitmodule "github.com/bitoro-network/chain/protocol/x/ratelimit/types"
	rewardsmodule "github.com/bitoro-network/chain/protocol/x/rewards/types"
	sendingtypes "github.com/bitoro-network/chain/protocol/x/sending/types"
	satypes "github.com/bitoro-network/chain/protocol/x/subaccounts/types"
	vestmodule "github.com/bitoro-network/chain/protocol/x/vest/types"
...
var (
	simChainId                         = "chain-protosapp-sim"
...
func BenchmarkFullAppSimulation(b *testing.B) {
	...
	bitoroApp.WithRandomlyGeneratedOperationsSimulationManager()
	...
	// https://github.com/bitoro-network/cosmos-sdk/blob/bitoro-fork-v0.47.0-rc2/x/auth/simulation/genesis.go#L26
	// For this reason, we need to register the `vesting` module interfaces so that the Genesis state of `auth` can be
	// marshaled properly.
	vestingtypes.RegisterInterfaces(bitoroApp.InterfaceRegistry())

	// Run randomized simulations
	_, simParams, simErr := simulation.SimulateFromSeed(
		b,
		os.Stdout,
		bitoroApp.GetBaseApp(),
		AppStateFn(bitoroApp.AppCodec(), bitoroApp.SimulationManager()),
		simtypes.RandomAccounts,
		CustomSimulationOperations(bitoroApp, bitoroApp.AppCodec(), config),
	...
	if err = simtestutil.CheckExportSimulation(bitoroApp, config, simParams); err != nil {
		b.Fatal(err)
	}
...
func TestFullAppSimulation(t *testing.T) {
	...
	bitoroApp.WithRandomlyGeneratedOperationsSimulationManager()
	require.Equal(t, "bitoroprotocol", bitoroApp.Name())
...
func TestAppStateDeterminism(t *testing.T) {
	...
	for i := 0; i < numSeeds; i++ {
		...
		for j := 0; j < numTimesToRunPerSeed; j++ {
			...
			appHash := bitoroApp.LastCommitID().Hash
...
func AppStateRandomizedFn(
	...
	appParams.GetOrGenerate(
		...
		func(r *rand.Rand) {
			...
			// (https://github.com/bitoro-network/cosmos-sdk/blob/93454d9f/x/auth/simulation/genesis.go#L38),
