	...
	return types.NewInterfaceRegistryWithOptions(types.InterfaceRegistryOptions{
		...
		SigningOptions: signing.Options{
			...
			CustomGetSigners: map[protoreflect.FullName]signing.GetSignersFunc{
				"bitoroprotocol.clob.MsgBatchCancel": getLegacyMsgSignerFn(
					[]string{"subaccount_id", "owner"},
				),
				"bitoroprotocol.clob.MsgCancelOrder": getLegacyMsgSignerFn(
					[]string{"order_id", "subaccount_id", "owner"},
				),
				"bitoroprotocol.clob.MsgPlaceOrder": getLegacyMsgSignerFn(
					[]string{"order", "order_id", "subaccount_id", "owner"},
				),
				"bitoroprotocol.sending.MsgCreateTransfer": getLegacyMsgSignerFn(
					[]string{"transfer", "sender", "owner"},
				),
				"bitoroprotocol.sending.MsgWithdrawFromSubaccount": getLegacyMsgSignerFn(
					[]string{"sender", "owner"},
				),
				"bitoroprotocol.vault.MsgDepositToMegavault": getLegacyMsgSignerFn(
					[]string{"subaccount_id", "owner"},
				),
				"bitoroprotocol.vault.MsgWithdrawFromMegavault": getLegacyMsgSignerFn(
					[]string{"subaccount_id", "owner"},
				),
				"bitoroprotocol.listing.MsgCreateMarketPermissionless": getLegacyMsgSignerFn(
					[]string{"subaccount_id", "owner"},
				),

				// App injected messages have no signers.
				"bitoroprotocol.bridge.MsgAcknowledgeBridges":  noSigners,
				"bitoroprotocol.clob.MsgProposedOperations":    noSigners,
				"bitoroprotocol.perpetuals.MsgAddPremiumVotes": noSigners,
				"bitoroprotocol.prices.MsgUpdateMarketPrices":  noSigners,
			},
		},
	})
}
