...
import (
	...
	testutil_json "github.com/Bitoro-Network/chain/protocol/testutil/json"
	keepertest "github.com/Bitoro-Network/chain/protocol/testutil/keeper"
	epochs_keeper "github.com/Bitoro-Network/chain/protocol/x/epochs/keeper"
	epoch_types "github.com/Bitoro-Network/chain/protocol/x/epochs/types"
	"github.com/Bitoro-Network/chain/protocol/x/perpetuals"
	perpetuals_keeper "github.com/Bitoro-Network/chain/protocol/x/perpetuals/keeper"
	prices_keeper "github.com/Bitoro-Network/chain/protocol/x/prices/keeper"
...
func TestAppModuleBasic_RegisterGRPCGatewayRoutes(t *testing.T) {
	...
	req, err := http.NewRequest("GET", "/bitoroprotocol/perpetuals/perpetual", nil)
	require.NoError(t, err)
	...
	recorder = httptest.NewRecorder()
	req, err = http.NewRequest("GET", "/bitoroprotocol/perpetuals/perpetual/0", nil)
	require.NoError(t, err)
	router.ServeHTTP(recorder, req)
	require.Contains(t, recorder.Body.String(), "no RPC client is defined in offline mode")
	...
	recorder = httptest.NewRecorder()
	req, err = http.NewRequest("GET", "/bitoroprotocol/perpetuals/foo/bar/baz", nil)
	require.NoError(t, err)
	router.ServeHTTP(recorder, req)
