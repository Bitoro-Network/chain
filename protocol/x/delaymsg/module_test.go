...
import (
	...
	delaymsg_keeper "github.com/Bitoro-Network/chain/protocol/x/delaymsg/keeper"
...
func TestAppModuleBasic_RegisterGRPCGatewayRoutes(t *testing.T) {
	...
	req, err := http.NewRequest("GET", "/bitoroprotocol/delaymsg/next_id", nil)
	require.NoError(t, err)
	...
	recorder = httptest.NewRecorder()
	req, err = http.NewRequest("GET", "/bitoroprotocol/delaymsg/message/0", nil)
	require.NoError(t, err)
	router.ServeHTTP(recorder, req)
	require.Contains(t, recorder.Body.String(), "no RPC client is defined in offline mode")
	...
	recorder = httptest.NewRecorder()
	req, err = http.NewRequest("GET", "/bitoroprotocol/delaymsg/block/message_ids/100", nil)
	require.NoError(t, err)
	router.ServeHTTP(recorder, req)
	require.Contains(t, recorder.Body.String(), "no RPC client is defined in offline mode")
	...
	recorder = httptest.NewRecorder()
	req, err = http.NewRequest("GET", "/bitoroprotocol/delaymsg/foo/bar/baz", nil)
	require.NoError(t, err)
	router.ServeHTTP(recorder, req)
