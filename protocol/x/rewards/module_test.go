...
import (
	...
	rewards_keeper "github.com/Bitoro-Network/chain/protocol/x/rewards/keeper"
...
func TestAppModuleBasic_RegisterGRPCGatewayRoutes(t *testing.T) {
	...
	req, err := http.NewRequest("GET", "/bitoroprotocol/rewards/params", nil)
	require.NoError(t, err)
	...
	recorder = httptest.NewRecorder()
	req, err = http.NewRequest("GET", "/bitoroprotocol/rewards/foo/bar/baz", nil)
	require.NoError(t, err)
	router.ServeHTTP(recorder, req)
