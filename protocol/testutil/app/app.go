...
// localbitoroprotocol Alice config/priv_validator_key.json.
...
// localbitoroprotocol Alice config/node_key.json.
...
func (tApp *TestApp) CheckTx(req abcitypes.RequestCheckTx) abcitypes.ResponseCheckTx {
	...
	// Note that the Bitoro fork of CometBFT explicitly excludes place and cancel order messages. See
	// https://github.com/bitoro-network/cometbft/blob/5e6c4b6/mempool/clist_mempool.go#L441
...
func launchValidatorInDir(
	...
	rootCmd := cmd.NewRootCmdWithInterceptors(
		...
		func(s string, appConfig *cmd.BitoroAppConfig) (string, *cmd.BitoroAppConfig) {
