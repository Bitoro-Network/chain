...
var (
	...
	invalidInnerMsgErr_Bitoro      = fmt.Errorf("Invalid nested msg for MsgExec: bitoro msg type")
...
func TestIsBitoroMsg_Invalid(t *testing.T) {
	allBitoroMsgs := lib.MergeAllMapsMustHaveDistinctKeys(
		appmsgs.AppInjectedMsgSamples,
		appmsgs.NormalMsgsBitoroCustom,
		appmsgs.InternalMsgSamplesBitoroCustom,
		map[string]sdk.Msg{
			// nolint:staticcheck
	...
	allMsgsMinusBitoro := lib.MergeAllMapsMustHaveDistinctKeys(appmsgs.AllowMsgs, appmsgs.DisallowMsgs)
	for key := range allBitoroMsgs {
		delete(allMsgsMinusBitoro, key)
	}
	allNonNilSampleMsgs := testmsgs.GetNonNilSampleMsgs(allMsgsMinusBitoro)

	for _, sampleMsg := range allNonNilSampleMsgs {
		t.Run(sampleMsg.Name, func(t *testing.T) {
			require.False(t, ante.IsBitoroMsg(sampleMsg.Msg))
		})
	}
}

func TestIsBitoroMsg_Valid(t *testing.T) {
	allBitoroMsgs := lib.MergeAllMapsMustHaveDistinctKeys(
		appmsgs.AppInjectedMsgSamples,
		appmsgs.NormalMsgsBitoroCustom,
		appmsgs.InternalMsgSamplesBitoroCustom,
	)
	allNonNilSampleMsgs := testmsgs.GetNonNilSampleMsgs(allBitoroMsgs)

	for _, sampleMsg := range allNonNilSampleMsgs {
		t.Run(sampleMsg.Name, func(t *testing.T) {
			require.True(t, ante.IsBitoroMsg(sampleMsg.Msg))
		})
	}
}
...
func TestValidateNestedMsg(t *testing.T) {
	tests := map[string]struct {
		...
		"Invalid MsgExec: bitoro custom msg": {
			msg:         &testmsgs.MsgExecWithBitoroMessage,
			expectedErr: invalidInnerMsgErr_Bitoro,
		},
