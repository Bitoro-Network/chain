...
const BITORO_MSG_PREFIX = "/" + constants.AppName
...
// IsBitoroMsg returns true if the given msg is a Bitoro custom msg.
func IsBitoroMsg(msg sdk.Msg) bool {
	return strings.HasPrefix(sdk.MsgTypeURL(msg), BITORO_MSG_PREFIX)
}
...
func validateInnerMsg(msg sdk.Msg) error {
	...
	for _, inner := range innerMsgs {
		...
		// 4. Reject nested bitoroprotocol messages in `MsgExec`.
		if _, ok := msg.(*authz.MsgExec); ok {
			...
			if IsBitoroMsg(inner) {
				return fmt.Errorf("Invalid nested msg for MsgExec: bitoro msg type")
			}
