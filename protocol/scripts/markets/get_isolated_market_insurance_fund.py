...
2. Build `bitoroprotocold` by running `make build` in the root directory of the protocol repository

    `cd chain/protocol && make clean && make build`
...
3. Run the script using the correct endpoint and binary path. Example:
    ...
    `python3 scripts/markets/get_isolated_market_insurance_fund.py \
        --endpoint=https://bitoro-ops-rest.kingnodes.com \
        --binary_path=/Users/a/chain/protocol/build/bitoroprotocold`
"""

def get_id_to_pair(base_endpoint_url):
    endpoint_url = base_endpoint_url + "/bitoroprotocol/prices/params/market"
...
def get_isolated_market_ids(base_endpoint_url):
    endpoint_url = base_endpoint_url + "/bitoroprotocol/perpetuals/perpetual"
...
def run_bitoroprotocold(command):
    try:
        ...
        print(f"Error running bitoroprotocold \n{e.stderr}")

    except FileNotFoundError:
        # Handle the case where the binary is not found
        print("Error: bitoroprotocold binary not found. Ensure you ran `make build`.")

def get_insurance_fund_address_for_markets(binary_path, market_ids):
    """
    This function takes a list of market_ids and runs the `bitoroprotocold` binary for each.
    ...
    for market_id in market_ids:
        ...
        address = run_bitoroprotocold(command)
        if not address:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--binary_path', required=True, help='The local path to the `bitoroprotocold` binary.')
