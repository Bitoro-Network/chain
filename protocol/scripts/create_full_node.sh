...
# This script is used to create a non validating full node for the bitoro protocol. 
# It will install all the necessary dependencies, download the bitoroprotocold binary, 
...
VERSION="v4.1.0"
WORKDIR=$HOME/.bitoroprotocol
CHAIN_ID="bitoro-mainnet-1"
...
"f04a77b92d0d86725cdb2d6b7a7eb0eda8c27089@bitoro-mainnet-seed.bwarelabs.com:36656",
...
"a9cae4047d5c34772442322b10ef5600d8e54900@bitoro-mainnet-seednode.allthatnode.com:26656",
"802607c6db8148b0c68c8a9ec1a86fd3ba606af6@64.227.38.88:26656",
"ebc272824924ea1a27ea3183dd0b9ba713494f83@bitoro-mainnet-seed.autostake.com:27366"
)
...
# Download bitoroprotocold binaries and initialize node
mkdir -p $WORKDIR
cd $WORKDIR
FILE_REGEX="bitoroprotocold-v[0-9]+\.[0-9]+\.[0-9]+-linux-$ARCH.tar.gz"
curl -s "https://api.github.com/repos/bitoro-network/chain/releases/tags/protocol/$VERSION" \
| grep -E $FILE_REGEX \
| cut -d : -f 2,3 \
| tr -d \" \
| wget -qi -
tar -xvf bitoroprotocold-v*-linux-$ARCH.tar.gz && rm bitoroprotocold-v*-linux-$ARCH.tar.gz
cd $WORKDIR/build
mv bitoroprotocold-*-linux-$ARCH bitoroprotocold
chmod +x bitoroprotocold
./bitoroprotocold init --chain-id=$CHAIN_ID $NODE_NAME

# Create cosmovisor directories and move binaries
mkdir -p $HOME/.bitoroprotocol/cosmovisor/genesis/bin
mkdir -p $HOME/.bitoroprotocol/cosmovisor/upgrades
mv bitoroprotocold $HOME/.bitoroprotocol/cosmovisor/genesis/bin/

# Update config
curl https://bitoro-rpc.lavenderfive.com/genesis | python3 -c 'import json,sys;print(json.dumps(json.load(sys.stdin)["result"]["genesis"], indent=2))' > $WORKDIR/config/genesis.json
sed -i 's/seeds = ""/seeds = "'"${SEED_NODES[*]}"'"/' $WORKDIR/config/config.toml

# Create Service
sudo tee /etc/systemd/system/bitoroprotocold.service > /dev/null << EOF
[Unit]
Description=bitoro node service
After=network-online.target

[Service]
User=$USER
ExecStart=/$HOME/go/bin/cosmovisor run start --non-validating-full-node=true
WorkingDirectory=$HOME/.bitoroprotocol
Restart=always
RestartSec=5
LimitNOFILE=4096
Environment="DAEMON_HOME=$HOME/.bitoroprotocol"
Environment="DAEMON_NAME=bitoroprotocold"
...
sudo systemctl enable bitoroprotocold


# Get snapshot
cd $WORKDIR
xml_content=$(curl -s "${BASE_SNAPSHOT_URL}")
file_key=$(echo "$xml_content" | grep -oP '<Key>bitoro/bitoro_\d+\.tar\.lz4</Key>' | head -1 | sed 's/<[^>]*>//g')
...
echo "To start the node run 'sudo systemctl start bitoroprotocold'"
echo "To stop the node run 'sudo systemctl stop bitoroprotocold'"