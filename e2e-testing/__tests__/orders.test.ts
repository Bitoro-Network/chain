...
import {
  BITORO_LOCAL_ADDRESS,
  BITORO_LOCAL_ADDRESS_2,
  BITORO_LOCAL_MNEMONIC,
  BITORO_LOCAL_MNEMONIC_2,
  orderDetails,
...
describe('orders', () => {
  it('test orders', async () => {
    ...
    const [wallet, wallet2] = await Promise.all([
      LocalWallet.fromMnemonic(BITORO_LOCAL_MNEMONIC, BECH32_PREFIX),
      LocalWallet.fromMnemonic(BITORO_LOCAL_MNEMONIC_2, BECH32_PREFIX),
    ]);
    ...
    const [ordersResponse, ordersResponse2] = await Promise.all([
      indexerClient.account.getSubaccountOrders(BITORO_LOCAL_ADDRESS,
        0,
        undefined,
        undefined,
        undefined,
        ...
        undefined,
        true),
      indexerClient.account.getSubaccountOrders(BITORO_LOCAL_ADDRESS_2,
        0,
        undefined,
        undefined,
        undefined,
    ...
    expect(ordersResponse[0]).toEqual(
      expect.objectContaining({
        subaccountId: SubaccountTable.uuid(BITORO_LOCAL_ADDRESS, 0),
        clobPairId: '0',
        side: 'BUY',
        size: '0.001',
        totalFilled: '0.0005',
    ...
    expect(ordersResponse2[0]).toEqual(
      expect.objectContaining({
        subaccountId: SubaccountTable.uuid(BITORO_LOCAL_ADDRESS_2, 0),
        clobPairId: '0',
        side: 'SELL',
        size: '0.0005',
        totalFilled: '0.0005',
    ...
    const [response, response2] = await Promise.all([
      indexerClient.account.getSubaccountPerpetualPositions(BITORO_LOCAL_ADDRESS, 0),
      indexerClient.account.getSubaccountPerpetualPositions(BITORO_LOCAL_ADDRESS_2, 0),
    ]);
  ...
  function validateOrders(data: any, socketClient: SocketClient): void {
    if (data.type === 'connected') {
      socketClient.subscribeToSubaccount(BITORO_LOCAL_ADDRESS, 0);
    } else if (data.type === 'subscribed') {
      expect(data.channel).toEqual('chain_subaccounts');
      expect(data.id).toEqual(`${BITORO_LOCAL_ADDRESS}/0`);
      expect(data.contents.subaccount).toEqual(
        expect.objectContaining({
          address: BITORO_LOCAL_ADDRESS,
          subaccountNumber: 0,
        }),
      );
