...
import { BITORO_LOCAL_ADDRESS, BITORO_LOCAL_MNEMONIC } from './helpers/constants';
import { connectAndValidateSocketClient } from './helpers/utils';

describe('transfers', () => {
  it('test deposit', async () => {
    connectAndValidateSocketClient(validateTransfers);
    const wallet = await LocalWallet.fromMnemonic(BITORO_LOCAL_MNEMONIC, BECH32_PREFIX);
    ...
    const response = await indexerClient.account.getSubaccountTransfers(BITORO_LOCAL_ADDRESS, 0);
    ...
    assetPosResp = await indexerClient.account.getSubaccountAssetPositions(BITORO_LOCAL_ADDRESS, 0);
    expect(assetPosResp).not.toBeNull();
