...
chain-proto-py-gen: proto-export-deps
	@rm -rf ./chain-proto-py/chain_proto
	@mkdir -p ./chain-proto-py/chain_proto
	@for dir in $(PROTO_DIRS); do \
		python3 -m grpc_tools.protoc \
		-I .proto-export-deps \
		--python_out=./chain-proto-py/chain_proto \
		--pyi_out=./chain-proto-py/chain_proto \
		--grpc_python_out=./chain-proto-py/chain_proto \
		$$(find ./$${dir} -type f -name '*.proto'); \
	done;
	perl -i -pe 's/^from (?!google\.protobuf)([^ ]*) import ([^ ]*)_pb2 as ([^ ]*)$$/from chain_proto.\1 import \2_pb2 as \3/' $$(find ./chain-proto-py/chain_proto -type f \( -name '*_pb2.py' -o -name '*_pb2_grpc.py' -o -name '*_pb2.pyi' -o -name '*_pb2_grpc.pyi' \))

.PHONY: proto-format proto-lint proto-check-bc-breaking proto-export proto-export-deps chain-proto-py-gen
