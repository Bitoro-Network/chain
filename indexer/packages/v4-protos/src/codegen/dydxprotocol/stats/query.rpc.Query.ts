...
export class QueryClientImpl implements Query {
  ...
  params(request: QueryParamsRequest = {}): Promise<QueryParamsResponse> {
    ...
    const promise = this.rpc.request("bitoroprotocol.stats.Query", "Params", data);
    return promise.then(data => QueryParamsResponse.decode(new _m0.Reader(data)));
  }

  statsMetadata(request: QueryStatsMetadataRequest = {}): Promise<QueryStatsMetadataResponse> {
    const data = QueryStatsMetadataRequest.encode(request).finish();
    const promise = this.rpc.request("bitoroprotocol.stats.Query", "StatsMetadata", data);
  ...
  globalStats(request: QueryGlobalStatsRequest = {}): Promise<QueryGlobalStatsResponse> {
    ...
    const promise = this.rpc.request("bitoroprotocol.stats.Query", "GlobalStats", data);
  ...
  userStats(request: QueryUserStatsRequest): Promise<QueryUserStatsResponse> {
    ...
    const promise = this.rpc.request("bitoroprotocol.stats.Query", "UserStats", data);
