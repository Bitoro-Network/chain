 ...
 */
function getSqlConversionForBitoroModelTypes(fieldName: string, type: string): string {
  switch (type) {
    case 'integer':
...
/**
 * Defines a `bitoro_to_jsonb` function for each of the models in SQL_TO_JSON_DEFINED_MODELS and
 * loads them in Postgres. This allows for plpgsql functions to invoke `bitoro_to_jsonb` on the
  ...
  await Promise.all(
    SQL_TO_JSON_DEFINED_MODELS.map(async (model) => {
      const sqlProperties: string[] = Object.entries(model.sqlToJsonConversions)
        .map(([key, value]) => `'${key}', ${getSqlConversionForBitoroModelTypes(key, value)}`);
      const sqlFn: string = `CREATE OR REPLACE FUNCTION bitoro_to_jsonb(row_t ${model.tableName}) RETURNS jsonb AS $$
BEGIN
...
$$ LANGUAGE plpgsql IMMUTABLE PARALLEL SAFE;`;
      return rawQuery(sqlFn, {}).catch((error) => {
        logger.error({
          ...
          message: `Failed to create or replace function bitoro_to_jsonb for model ${model.tableName}.`,
          error,
        });
        throw error;
      });
...
/**
 * Drops all functions named bitoro_.* from the database.
 */
async function dropAllBitoroFunctions() {
