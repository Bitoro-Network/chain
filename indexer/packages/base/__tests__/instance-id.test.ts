...
import { asMock } from '@bitoroprotocol-indexer/dev';
...
describe('instance-id', () => {
  describe('setInstanceId', () => {
    ...
    const ecsUrl = config.ECS_CONTAINER_METADATA_URI;

    beforeEach(() => {
      config.ECS_CONTAINER_METADATA_URI = ecsUrl;
      resetForTests();
      jest.resetAllMocks();
    ...
    it('should set instance id to task ARN in staging', async () => {
      ...
      config.ECS_CONTAINER_METADATA_URI = 'url';
      await setInstanceId();
