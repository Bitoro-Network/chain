...
AWS_PROFILE=bitoro-$env aws ecr get-login-password --region ap-northeast-1 | docker login --username AWS --password-stdin $account.dkr.ecr.ap-northeast-1.amazonaws.com
...
AWS_PROFILE=bitoro-$env orb deploy $service $tag
