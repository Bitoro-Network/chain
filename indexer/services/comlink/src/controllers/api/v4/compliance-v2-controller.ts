import { logger, stats, TooManyRequestsError } from '@bitoroprotocol-indexer/base';
import { CountryHeaders, isRestrictedCountryHeaders, isWhitelistedAddress } from '@bitoroprotocol-indexer/compliance';
import {
  ComplianceReason,
  ComplianceStatus,
...
import { BITORO_ADDRESS_PREFIX } from '../../../lib/constants';
    ...
    async (req: express.Request, res: express.Response) => {
      ...
      try {
        if (!address.startsWith(BITORO_ADDRESS_PREFIX)) {
          return create4xxResponse(
            res,
            `Address ${address} is not a bitoro address`,
          );
        }
