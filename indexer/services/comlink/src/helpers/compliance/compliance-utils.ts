...
import { CountryHeaders, isRestrictedCountryHeaders } from '@bitoroprotocol-indexer/compliance';
import { ComplianceReason } from '@bitoroprotocol-indexer/postgres';
import { verifyADR36Amino } from '@keplr-wallet/cosmos';
import express from 'express';
import { DateTime } from 'luxon';

import { GEOBLOCK_REQUEST_TTL_SECONDS, BITORO_ADDRESS_PREFIX } from '../../lib/constants';
...
function generateAddress(pubkeyArray: Uint8Array): string {
  return toBech32('btoro', ripemd160(sha256(pubkeyArray)));
}
...
export async function validateSignature(
  ...
  if (!address.startsWith(BITORO_ADDRESS_PREFIX)) {
    return create4xxResponse(
      res,
      `Address ${address} is not a valid Bitoro address`,
    );
  }
...
export function validateSignatureKeplr(
  ...
  const isVerified = verifyADR36Amino(
    'btoro', address, messageToSign, pubKeyUint, signedMessageUint, 'secp256k1',
  );
