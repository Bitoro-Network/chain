import { isValidLanguageCode } from '@bitoroprotocol-indexer/notifications';
import {
...
export const CheckSubaccountSchema = checkSchema({
  address: {
    ...
    errorMessage: 'address must be a valid bitoro address',
  },
  subaccountNumber: {
    in: ['params', 'query'],
    isInt: {
...
export function isValidBitoroAddress(address: string): boolean {
  // An address is valid if it starts with `btoro1` and is Bech32 format.
  return address.startsWith('btoro1') && (verifyIsBech32(address) === undefined);
}
