...
import { logger, stats, STATS_NO_SAMPLING } from '@bitoroprotocol-indexer/base';
import { BLOCK_HEIGHT_WEBSOCKET_MESSAGE_VERSION, KafkaTopics } from '@bitoroprotocol-indexer/kafka';
import {
  storeHelpers,
...
import {
  ...
  BitoroIndexerSubtypes, EventMessage, EventProtoWithTypeAndVersion, GroupedEvents, SKIPPED_EVENT_SUBTYPE,
} from './types';

const TXN_EVENT_SUBTYPE_VERSION_TO_VALIDATOR_MAPPING: Record<string, ValidatorInitializer> = {
  [serializeSubtypeAndVersion(BitoroIndexerSubtypes.ORDER_FILL.toString(), 1)]: OrderFillValidator,
  [serializeSubtypeAndVersion(BitoroIndexerSubtypes.SUBACCOUNT_UPDATE.toString(), 1)]: SubaccountUpdateValidator,
  [serializeSubtypeAndVersion(BitoroIndexerSubtypes.TRANSFER.toString(), 1)]: TransferValidator,
  [serializeSubtypeAndVersion(BitoroIndexerSubtypes.MARKET.toString(), 1)]: MarketValidator,
  [serializeSubtypeAndVersion(BitoroIndexerSubtypes.STATEFUL_ORDER.toString(), 1)]: StatefulOrderValidator,
  [serializeSubtypeAndVersion(BitoroIndexerSubtypes.ASSET.toString(), 1)]: AssetValidator,
  [serializeSubtypeAndVersion(BitoroIndexerSubtypes.PERPETUAL_MARKET.toString(), 1)]: PerpetualMarketValidator,
  [serializeSubtypeAndVersion(BitoroIndexerSubtypes.PERPETUAL_MARKET.toString(), 2)]: PerpetualMarketValidator,
  [serializeSubtypeAndVersion(BitoroIndexerSubtypes.PERPETUAL_MARKET.toString(), 3)]: PerpetualMarketValidator,
  [serializeSubtypeAndVersion(BitoroIndexerSubtypes.LIQUIDITY_TIER.toString(), 1)]: LiquidityTierValidator,
  [serializeSubtypeAndVersion(BitoroIndexerSubtypes.UPDATE_PERPETUAL.toString(), 1)]: UpdatePerpetualValidator,
  [serializeSubtypeAndVersion(BitoroIndexerSubtypes.UPDATE_PERPETUAL.toString(), 2)]: UpdatePerpetualValidator,
  [serializeSubtypeAndVersion(BitoroIndexerSubtypes.UPDATE_PERPETUAL.toString(), 3)]: UpdatePerpetualValidator,
  [serializeSubtypeAndVersion(BitoroIndexerSubtypes.UPDATE_CLOB_PAIR.toString(), 1)]: UpdateClobPairValidator,
  [serializeSubtypeAndVersion(BitoroIndexerSubtypes.DELEVERAGING.toString(), 1)]: DeleveragingValidator,
  [serializeSubtypeAndVersion(BitoroIndexerSubtypes.LIQUIDITY_TIER.toString(), 2)]: LiquidityTierValidatorV2,
  [serializeSubtypeAndVersion(BitoroIndexerSubtypes.REGISTER_AFFILIATE.toString(), 1)]: RegisterAffiliateValidator,
  [serializeSubtypeAndVersion(BitoroIndexerSubtypes.UPSERT_VAULT.toString(), 1)]: UpsertVaultValidator,
};

const BLOCK_EVENT_SUBTYPE_VERSION_TO_VALIDATOR_MAPPING: Record<string, ValidatorInitializer> = {
  [serializeSubtypeAndVersion(BitoroIndexerSubtypes.FUNDING.toString(), 1)]: FundingValidator,
  [serializeSubtypeAndVersion(BitoroIndexerSubtypes.TRADING_REWARD.toString(), 1)]: TradingRewardsValidator,
  [serializeSubtypeAndVersion(BitoroIndexerSubtypes.STATEFUL_ORDER.toString(), 1)]: StatefulOrderValidator,
  [serializeSubtypeAndVersion(BitoroIndexerSubtypes.UPSERT_VAULT.toString(), 1)]: UpsertVaultValidator,
};
...
export class BlockProcessor {
  ...
  private validateAndAddHandlerForEvent(
    ...
    _.map(handlers, (handler: Handler<EventMessage>) => {
      if (SYNCHRONOUS_SUBTYPES.includes(eventProto.type as BitoroIndexerSubtypes)) {
  ...
  private async processEvents(): Promise<KafkaPublisher> {
    ...
    try {
      const result: pg.QueryResult = await storeHelpers.rawQuery(
        'SELECT bitoro_block_processor(?) AS result;',
        {
          txId: this.txId,
          bindings: [JSON.stringify(this.sqlBlock)],
          sqlOptions: { name: 'bitoro_block_processor' },
        },
