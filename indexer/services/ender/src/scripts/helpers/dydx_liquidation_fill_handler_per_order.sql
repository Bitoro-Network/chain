CREATE OR REPLACE FUNCTION bitoro_liquidation_fill_handler_per_order(
...
/**
  ...
  Returns: JSON object containing fields:
    - order: The updated order in order-model format (https://github.com/Bitoro-Network/chain/blob/9ed26bd/indexer/packages/postgres/src/models/order-model.ts).
        Only returned if field == 'makerOrder'.
    - fill: The updated fill in fill-model format (https://github.com/Bitoro-Network/chain/blob/9ed26bd/indexer/packages/postgres/src/models/fill-model.ts).
    - perpetual_market: The perpetual market for the order in perpetual-market-model format (https://github.com/Bitoro-Network/chain/blob/9ed26bd/indexer/packages/postgres/src/models/perpetual-market-model.ts).
    - perpetual_position: The updated perpetual position in perpetual-position-model format (https://github.com/Bitoro-Network/chain/blob/9ed26bd/indexer/packages/postgres/src/models/perpetual-position-model.ts).
...
BEGIN
    ...
    perpetual_market_record = bitoro_get_perpetual_market_for_clob_pair(clob_pair_id);
    market_record = bitoro_get_market_for_id(perpetual_market_record."marketId");
    ...
    */
    fill_amount = bitoro_trim_scale(bitoro_from_jsonlib_long(event_data->'fillAmount') *
                                  power(10, perpetual_market_record."atomicResolution")::numeric);
    maker_price = bitoro_trim_scale(bitoro_from_jsonlib_long(maker_order->'subticks') *
    ...
    total_filled = bitoro_trim_scale(bitoro_get_total_filled(fill_liquidity, event_data) *
                                   power(10, perpetual_market_record."atomicResolution")::numeric);
    fee = bitoro_trim_scale(bitoro_get_fee(fill_liquidity, event_data) *
                          power(10, asset_record."atomicResolution")::numeric);
    affiliate_rev_share = bitoro_trim_scale(bitoro_from_jsonlib_long(event_data->'affiliateRevShare') *
                                    power(10, asset_record."atomicResolution")::numeric);
    order_price = bitoro_trim_scale(bitoro_from_jsonlib_long(order_->'subticks') *
                                  power(10, perpetual_market_record."quantumConversionExponent" +
                                            asset_record."atomicResolution" -
                                            perpetual_market_record."atomicResolution")::numeric);

    IF field = 'makerOrder' THEN
        order_uuid = bitoro_uuid_from_order_id(order_->'orderId');
        subaccount_uuid = bitoro_uuid_from_subaccount_id(jsonb_extract_path(order_, 'orderId', 'subaccountId'));
        order_client_metadata = (order_->'clientMetadata')::bigint;
        order_side = bitoro_from_protocol_order_side(order_->'side');
    ELSE
        order_uuid = NULL;
        subaccount_uuid = bitoro_uuid_from_subaccount_id(jsonb_extract_path(order_, 'liquidated'));
    ...
    IF field = 'makerOrder' THEN
        order_size = bitoro_trim_scale(bitoro_from_jsonlib_long(order_->'quantums') *
                                     power(10, perpetual_market_record."atomicResolution")::numeric);
        ...
        order_record."timeInForce" = bitoro_from_protocol_time_in_force(order_->'timeInForce');
        ...
        IF FOUND THEN
            ...
            order_record."status" = bitoro_get_order_status(total_filled, order_record.size, 'NOT_CANCELED', order_record."orderFlags", order_record."timeInForce");
        ...
        ELSE
            ...
            order_record."status" = bitoro_get_order_status(fill_amount, order_size, 'NOT_CANCELED', order_record."orderFlags", order_record."timeInForce");
    ...
    event_id = bitoro_event_id_from_parts(
    ...
    VALUES (bitoro_uuid_from_fill_event_parts(event_id, fill_liquidity),
            subaccount_uuid,
            order_side,
            fill_liquidity,
            fill_type,
            ...
            fill_amount,
            maker_price,
            bitoro_trim_scale(fill_amount * maker_price),
            event_id,
            transaction_hash,
            block_time,
            block_height,
    ...
    BEGIN
        IF bitoro_perpetual_position_and_order_side_matching(
                perpetual_position_record."side", order_side) THEN
            sum_open = bitoro_trim_scale(perpetual_position_record."sumOpen" + fill_amount);
            entry_price = bitoro_get_weighted_average(
        ...
        ELSE
            sum_close = bitoro_trim_scale(perpetual_position_record."sumClose" + fill_amount);
            exit_price = bitoro_get_weighted_average(
    ...
    IF field = 'makerOrder' THEN
        RETURN jsonb_build_object(
                'order',
                bitoro_to_jsonb(order_record),
                'fill',
                bitoro_to_jsonb(fill_record),
                'perpetual_market',
                bitoro_to_jsonb(perpetual_market_record),
                'market',
                bitoro_to_jsonb(market_record),
                'perpetual_position',
                bitoro_to_jsonb(perpetual_position_record)
            );
    ELSE
        RETURN jsonb_build_object(
                'fill',
