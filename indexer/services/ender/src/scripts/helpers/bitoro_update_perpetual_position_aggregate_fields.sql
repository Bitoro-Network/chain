CREATE OR REPLACE FUNCTION bitoro_update_perpetual_position_aggregate_fields(
...
BEGIN
    ...
    IF bitoro_perpetual_position_and_order_side_matching(perpetual_position_record."side", side) THEN
        sum_open := bitoro_trim_scale(perpetual_position_record."sumOpen" + size);
    ...
    ELSE
        sum_close := bitoro_trim_scale(perpetual_position_record."sumClose" + size);
