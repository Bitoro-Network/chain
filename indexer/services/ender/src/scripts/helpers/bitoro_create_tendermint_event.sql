CREATE OR REPLACE FUNCTION bitoro_create_tendermint_event(
...
BEGIN
    transaction_idx := bitoro_tendermint_event_to_transaction_index(event);
    event_id := bitoro_event_id_from_parts(CAST(block_height AS int), transaction_idx, CAST(event->>'eventIndex' AS int));
