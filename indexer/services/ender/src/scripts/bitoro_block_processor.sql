CREATE OR REPLACE FUNCTION bitoro_block_processor(block jsonb) RETURNS jsonb AS $$
/**
...
DECLARE
    ...
    rval_to_merge jsonb[];
BEGIN
    PERFORM bitoro_create_initial_rows_for_tendermint_block(block_height, block_time, block->'txHashes', block->'events');
    ...
    IF NOT block_height = 0 THEN
        rval = bitoro_block_processor_unordered_handlers(block);
        rval_to_merge = bitoro_block_processor_ordered_handlers(block);
    ELSE
        rval = bitoro_block_processor_ordered_handlers(block);
        rval_to_merge = bitoro_block_processor_unordered_handlers(block);
    END IF;
