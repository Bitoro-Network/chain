...
BEGIN
    ...
    ELSE
        liquidity_tier_record."openInterestLowerCap" = bitoro_trim_scale(bitoro_from_jsonlib_long(event_data->'openInterestLowerCap') *
    ...
    ELSE
        liquidity_tier_record."openInterestUpperCap" = bitoro_trim_scale(bitoro_from_jsonlib_long(event_data->'openInterestUpperCap') *
                                  power(10, QUOTE_CURRENCY_ATOMIC_RESOLUTION)::numeric);
    END IF;
    ...
    RETURN jsonb_build_object(
        'liquidity_tier',
        bitoro_to_jsonb(liquidity_tier_record)
    );
END;
$$ LANGUAGE plpgsql;