CREATE OR REPLACE FUNCTION bitoro_block_processor_ordered_handlers(block jsonb) RETURNS jsonb[] AS $$
/**
        ...
        CASE event_->'subtype'
            WHEN '"market"'::jsonb THEN
                IF event_data->'priceUpdate' IS NOT NULL THEN
                    rval[i] = bitoro_market_price_update_handler(block_height, block_time, event_data);
                ELSIF event_data->'marketCreate' IS NOT NULL THEN
                    rval[i] = bitoro_market_create_handler(event_data);
                ELSIF event_data->'marketModify' IS NOT NULL THEN
                    rval[i] = bitoro_market_modify_handler(event_data);
                ELSE
                    RAISE EXCEPTION 'Unknown market event %', event_;
                END IF;
            WHEN '"asset"'::jsonb THEN
                rval[i] = bitoro_asset_create_handler(event_data);
            WHEN '"perpetual_market"'::jsonb THEN
                CASE (event_->'version')::int
                    WHEN 1 THEN
                        rval[i] = bitoro_perpetual_market_v1_handler(event_data);
                    WHEN 2 THEN
                        rval[i] = bitoro_perpetual_market_v2_handler(event_data);
                    WHEN 3 THEN
                        rval[i] = bitoro_perpetual_market_v3_handler(event_data);
                    ELSE
                        NULL;
                END CASE;
            WHEN '"liquidity_tier"'::jsonb THEN
                rval[i] = bitoro_liquidity_tier_handler(event_data);
            WHEN '"update_perpetual"'::jsonb THEN
                CASE (event_->'version')::int
                    WHEN 1 THEN
                        rval[i] = bitoro_update_perpetual_v1_handler(event_data);
                    WHEN 2 THEN
                        rval[i] = bitoro_update_perpetual_v2_handler(event_data);
                    WHEN 3 THEN
                        rval[i] = bitoro_update_perpetual_v3_handler(event_data);
                    ELSE
                        NULL;
                END CASE;
            WHEN '"update_clob_pair"'::jsonb THEN
                rval[i] = bitoro_update_clob_pair_handler(event_data);
            WHEN '"funding_values"'::jsonb THEN
                rval[i] = bitoro_funding_handler(block_height, block_time, event_data, event_index, transaction_index);
            WHEN '"upsert_vault"'::jsonb THEN
                rval[i] = bitoro_vault_upsert_handler(block_time, event_data);
