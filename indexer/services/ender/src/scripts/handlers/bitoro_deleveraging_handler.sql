CREATE OR REPLACE FUNCTION bitoro_deleveraging_handler(
...
/**
  ...
  Returns: JSON object containing fields:
    - liquidated_fill: The created liquidated fill in fill-model format (https://github.com/bitoro-network/chain/blob/9ed26bd/indexer/packages/postgres/src/models/fill-model.ts).
    - offsetting_fill: The created offsetting fill in fill-model format (https://github.com/bitoro-network/chain/blob/9ed26bd/indexer/packages/postgres/src/models/fill-model.ts).
    - perpetual_market: The perpetual market for the deleveraging in perpetual-market-model format (https://github.com/bitoro-network/chain/blob/9ed26bd/indexer/packages/postgres/src/models/perpetual-market-model.ts).
    - liquidated_perpetual_position: The updated liquidated perpetual position in perpetual-position-model format (https://github.com/bitoro-network/chain/blob/9ed26bd/indexer/packages/postgres/src/models/perpetual-position-model.ts).
    - offsetting_perpetual_position: The updated offsetting perpetual position in perpetual-position-model format (https://github.com/bitoro-network/chain/blob/9ed26bd/indexer/packages/postgres/src/models/perpetual-position-model.ts).
...
BEGIN
    ...
    */
    size = bitoro_trim_scale(bitoro_from_jsonlib_long(event_data->'fillAmount') *
                                 power(10, perpetual_market_record."atomicResolution")::numeric);
    quote_amount = bitoro_trim_scale(bitoro_from_jsonlib_long(event_data->'totalQuoteQuantums') *
                                  power(10, QUOTE_CURRENCY_ATOMIC_RESOLUTION)::numeric);
    price = bitoro_trim_scale(quote_amount / size);

    liquidated_subaccount_uuid = bitoro_uuid_from_subaccount_id(event_data->'liquidated');
    offsetting_subaccount_uuid = bitoro_uuid_from_subaccount_id(event_data->'offsetting');
    ...
    VALUES (bitoro_uuid_from_fill_event_parts(event_id, 'TAKER'),
    ...
    INSERT INTO fills
        ("id", "subaccountId", "side", "liquidity", "type", "clobPairId", "size", "price", "quoteAmount",
         "eventId", "transactionHash", "createdAt", "createdAtHeight", "fee", "affiliateRevShare")
    VALUES (bitoro_uuid_from_fill_event_parts(event_id, 'MAKER'),
    ...
    liquidated_perpetual_position_record = bitoro_update_perpetual_position_aggregate_fields(
        liquidated_subaccount_uuid,
        perpetual_id,
        liquidated_side,
        size,
        price);
    offsetting_perpetual_position_record = bitoro_update_perpetual_position_aggregate_fields(
        offsetting_subaccount_uuid,
        perpetual_id,
        offsetting_side,
        size,
    ...
    RETURN jsonb_build_object(
            'liquidated_fill',
            bitoro_to_jsonb(liquidated_fill_record),
            'offsetting_fill',
            bitoro_to_jsonb(offsetting_fill_record),
            'perpetual_market',
            bitoro_to_jsonb(perpetual_market_record),
            'market',
            bitoro_to_jsonb(market_record),
            'liquidated_perpetual_position',
            bitoro_to_jsonb(liquidated_perpetual_position_record),
            'offsetting_perpetual_position',
            bitoro_to_jsonb(offsetting_perpetual_position_record)
        );
END;
$$ LANGUAGE plpgsql;
