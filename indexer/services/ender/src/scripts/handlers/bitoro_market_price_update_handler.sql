CREATE OR REPLACE FUNCTION bitoro_market_price_update_handler(block_height int, block_time timestamp, event_data jsonb) RETURNS jsonb AS $$
/**
  Parameters:
  ...
  Returns: JSON object containing fields:
    - market: The updated market in market-model format (https://github.com/bitoro-network/chain/blob/9ed26bd/indexer/packages/postgres/src/models/market-model.ts).
    - oracle_price: The created oracle price in oracle-price-model format (https://github.com/bitoro-network/chain/blob/9ed26bd/indexer/packages/postgres/src/models/oracle-price-model.ts).
...
BEGIN
    ...
    oracle_price = bitoro_trim_scale(
        (bitoro_from_jsonlib_long(event_data->'priceUpdate'->'priceWithExponent') *
    ...
    oracle_price_record."id" = bitoro_uuid_from_oracle_price_parts(market_record_id, block_height);
    ...
    RETURN jsonb_build_object(
        ...
        'oracle_price',
        bitoro_to_jsonb(oracle_price_record)
    );
END;
$$ LANGUAGE plpgsql;