...
describe('syncHandler', () => {
  describe('addHandler/process', () => {
    it('successfully adds handler', async () => {
      ...
      syncHandlers.addHandler(BitoroIndexerSubtypes.MARKET, firstHandler);
      syncHandlers.addHandler(BitoroIndexerSubtypes.ASSET, secondHandler);
      // should be ignored, because transfers are not handled by syncHandlers
      syncHandlers.addHandler(BitoroIndexerSubtypes.TRANSFER, handlerNotInvoked);
