...
import { getOrderIdHash, ORDER_FLAG_CONDITIONAL } from '@bitoroprotocol-indexer/chain-proto-parser';
import {
  LiquidationOrderV1,
  MarketMessage,
  IndexerOrder,
...
import { BitoroIndexerSubtypes, VulcanMessage } from '../../src/lib/types';
...
export function createKafkaMessageFromOrderFillEvent({
  ...
  const events: IndexerTendermintEvent[] = [
    createIndexerTendermintEvent(
      BitoroIndexerSubtypes.ORDER_FILL,
...
export function createKafkaMessageFromDeleveragingEvent({
  ...
  const events: IndexerTendermintEvent[] = [
    createIndexerTendermintEvent(
      BitoroIndexerSubtypes.DELEVERAGING,
