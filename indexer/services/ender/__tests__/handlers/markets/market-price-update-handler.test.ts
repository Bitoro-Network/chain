...
import { BitoroIndexerSubtypes, MarketPriceUpdateEventMessage } from '../../../src/lib/types';
import {
  defaultHeight,
...
import { MarketEventV1, IndexerTendermintBlock, IndexerTendermintEvent } from '@bitoroprotocol-indexer/chain-protos';
...
describe('marketPriceUpdateHandler', () => {
  ...
  it('fails when no market exists', async () => {
    ...
    expect(loggerCrit).toHaveBeenCalledWith(expect.objectContaining({
      at: expect.stringContaining('PL/pgSQL function bitoro_market_price_update_handler('),
