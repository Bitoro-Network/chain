...
import { IndexerTendermintBlock, IndexerTendermintEvent, MarketEventV1 } from '@bitoroprotocol-indexer/chain-protos';
import { KafkaMessage } from 'kafkajs';
import { onMessage } from '../../../src/lib/on-message';
import { BitoroIndexerSubtypes, MarketCreateEventMessage } from '../../../src/lib/types';
import {
  defaultHeight,
  defaultMarketCreate,
...
describe('marketCreateHandler', () => {
  ...
  it('errors when attempting to create an existing market', async () => {
    ...
    expect(loggerCrit).toHaveBeenCalledWith(expect.objectContaining({
      at: expect.stringContaining('PL/pgSQL function bitoro_market_create_handler('),
