...
import { updateOnBatchFunction, updateOnMessageFunction } from '@bitoroprotocol-indexer/kafka';
import {
  Batch,
  EachBatchPayload,
  KafkaMessage,
...
export class MessageForwarder {
  ...
  public forwardMessage(message: MessageToForward): void {
    ...
    //   if (message.channel !== Channel.CHAIN_ORDERBOOK ||
    //       (
    //         // Don't log orderbook messages unless enabled
    //         message.channel === Channel.CHAIN_ORDERBOOK && config.ENABLE_ORDERBOOK_LOGS
    //       )
    //   ) {
    //     logger.debug({
    //       at: 'message-forwarder#forwardMessage',
